...
module.exports = class extends Base {
  indexAction() {
    ...
    return _asyncToGenerator(function*() {
      const goodsOnsale = yield _this
        .model("goods")
        .where({ is_on_sale: 1, is_delete: 0 })
        .count();
      const orderToDelivery = yield _this
        .model("order")
  ...
  mainAction() {
    ...
    return _asyncToGenerator(function*() {
      ...
      if (index == 0) {
        ...
        addCart = yield _this3
          .model("cart")
          .where({ is_delete: 0, add_time: [">", todayTimeStamp] })
          .count();
        addOrderNum = yield _this3
          .model("order")
          .where({
            is_delete: 0,
